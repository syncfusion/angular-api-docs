...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/angular-api-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/angular-api-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '') {
                ...
                if (fs.existsSync('../angular-api-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/angular-api-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../angular-api-docs/` + changedFileName, `../../../gitlabRepo/angular-api-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/angular-api-docs/')) {
                            shelljs.cp('-rf', `../angular-api-docs/` + changedFileName, `../../../gitlabRepo/angular-api-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/angular-api-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/angular-api-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/angular-api-docs`);
